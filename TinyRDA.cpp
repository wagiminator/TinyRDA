...
// based on a project by Maarten Janssen (https://hackaday.io/project/9009-arduino-radio-with-rds)
// modified to work with TinyI2C library 2020 by Stefan Wagner
...
bool Radio::init() {
	...
	TinyI2C.init();
	bool success = TinyI2C.start(I2C_INDEX, 0);
	TinyI2C.stop();
...
void Radio::updateStatus() {
	TinyI2C.start(I2C_SEQ, (6 * 2) );
	for (int i = RADIO_REG_A; i <= RADIO_REG_F; i ++) {
		regs[i] = (uint16_t)(TinyI2C.read() << 8) | TinyI2C.read();
	}
	TinyI2C.stop();
...
void Radio::sendRegister(byte reg) {
	TinyI2C.start(I2C_INDEX, 0);
	TinyI2C.write(reg);
	TinyI2C.write(regs[reg] >> 8);
	TinyI2C.write(regs[reg] & 0xFF);
	TinyI2C.stop();
}
